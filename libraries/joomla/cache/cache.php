...
{
	/**
	 * @var    int  Pattern that matches with prefix.
	 * @since  13.1
	 */
	const TYPE_PREFIX = 1;

	/**
	 * @var    int  Pattern that matches with regular expression.
	 * @since  13.1
	 */
	const TYPE_REGEX  = 2;
	...
	/**
	 * Clean the cache of items specified by given pattern.
	 *
	 * @param   string   $pattern  The pattern to use to empty particular items if supported by the cache layer.
	 ...
	 * @since   13.1
	 */
	public function clean($pattern = '', $type = self::TYPE_PREFIX)
	{
		$this->flush($pattern, $type);
	 ...
	 * @throws  RuntimeException
	 */
	abstract protected function flush($pattern = '', $type = self::TYPE_PREFIX);
	...
	{
		return JCacheLegacy::getInstance($type, $options);
	}
	...
	{
		return JCacheLegacy::getWorkarounds($data, $options);
	}
	...
	{
		return JCacheLegacy::setWorkarounds($data, $options);
	}
	...
	{
		return JCacheLegacy::addIncludePath($path);
	}
}
